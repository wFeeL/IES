...
def test_recommend_and_quick_pages_render_forecast_only_blocks(client):
    ...
    assert "Целевая" in quick_html
    assert "Потолок" in quick_html
    assert "Чистая прибыль после выбранной цены" in quick_html
    assert "Остаток бюджета после выбранной цены" in quick_html
    ...
    assert "Подтвердить покупку" in quick_html
