...
5. Перейти в `Forecast`, загрузить CSV и проверить блоки `Raw CSV columns`, `Mapping`, `Unsupported / unused`.
...
- Рабочая цена (`working_bid`) едина для стратегии, таблиц, карточки лота и quick auction и строится из новой valuation-модели:
  ...
  - `working_bid` - пользовательская цена для покупки, уже учитывающая риск-band, бюджет, portfolio synergy и system fit.
...
- В forecast UI данные разведены по слоям:
  ...
  - `Unsupported / unused` - колонки CSV, которые не участвуют в модели.
...
1. Загрузить CSV прогноза и проверить `Raw CSV columns`, `Mapping`, `Unsupported / unused`.
