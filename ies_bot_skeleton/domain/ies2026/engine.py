...
from .config import ies2026_config, scenario_coefficients
...
def _consumer_tariff(obj: EnergyObject) -> float:
    ...
    return _as_float(
        params.get("tariff_rub_per_mw_tick", params.get("service_tariff_rub_per_mw_tick", 0.0)),
        0.0,
    )
...
def _solar_output(obj: EnergyObject, tick: ForecastTick, scenario: Dict[str, float], config: Dict[str, Any]) -> float:
    ...
    capacity = _as_float(params.get("generation_mw", params.get("capacity_mw", defaults.get("default_capacity_mw", 22.0))), 22.0)
...
def _estimate_declared_sales(
    *,
    ...
    ramp_limit = _as_float(market_cfg.get("sale_ramp_limit_mw", 8.0), 8.0)
    ...
    for tick in dataset.ticks:
        ...
        cap = _as_float(tick.extra_market.get("bid_cap_mw", market_cfg.get("bid_cap_mw", 120.0)), 120.0)
        current = min(cap, max(0.0, min(raw_surplus, previous + ramp_limit)))
...
def _simulate(
    *,
    ...
    if declared_sales is None:
        declared_sales = _estimate_declared_sales(
            objects=objects,
            dataset=dataset,
            topology=topology,
            config=config,
    ...
    for idx, tick in enumerate(dataset.ticks):
        ...
        for obj in objects:
            ...
            if obj.category == "consumer":
                ...
                tick_consumer_revenue += demand * _consumer_tariff(obj)
                if gross > demand:
                    tick_unserved_penalty += (gross - demand) * _consumer_penalty_rate(obj, config)
                continue
        ...
        if storage_cfg["capacity"] > 0.0:
            ...
            declared_sale = float(declared_sales[idx] if idx < len(declared_sales) else 0.0)
            ...
            elif declared_sale > max(0.0, net_root) and soc > reserve_floor:
                discharge = min(
                    declared_sale - max(0.0, net_root),
                    storage_cfg["discharge"],
                    max(0.0, soc - reserve_floor),
                )
                if discharge > 0:
        ...
        declared_sale = float(declared_sales[idx] if idx < len(declared_sales) else 0.0)
        shortfall = max(0.0, declared_sale - max(0.0, net_root))
        actual_high_sale = min(max(0.0, net_root), declared_sale)
        low_price_sale = max(0.0, max(0.0, net_root) - actual_high_sale)
        market_revenue = actual_high_sale * sell_price + low_price_sale * sell_price * low_sale_factor
        purchase_cost = max(0.0, -net_root) * buy_price
        ...
        market_events.append(
            MarketBid(
                ...
                low_price=sell_price * low_sale_factor,
    ...
    totals.flexibility_credit += totals.storage.arbitrage + totals.storage.balancing + totals.storage.reserve
...
def _consumer_tariff_break_even(
    *,
    ...
    working = _clone_objects([*base_objects, *candidate_objects])
    _, planned = plan_network(
        existing_objects=[obj for obj in working if not obj.is_candidate],
        candidate_objects=[obj for obj in working if obj.is_candidate],
    )
    direct_report = _direct_delta(
    ...
    if direct_report["expected_delta_profit"] <= 0.0:
        return max(0.0, current_tariff)

    low = 0.0
    high = max(current_tariff * 1.5, 2.0)
    for _ in range(18):
        mid = (low + high) / 2.0
...
def evaluate_candidate_bundle(
    *,
    ...
    assumptions = [
        ...
        "Биржевой экспорт использует антидемпинговый лимит роста заявленного объёма и отдельную цену на непроданный остаток.",
    ]
    if direction == "descending_consumer_tariff":
        explanation = (
            f"Лот рассматривается как потребительский объект с аукционом на понижение тарифа. "
            f"Безубыточный тариф {break_even:.2f}, рекомендуемый оффер {recommended:.2f}. "
